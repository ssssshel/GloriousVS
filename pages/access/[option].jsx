...
const UserAccess = () => {
	...
	async function registerUser(email, password, role) {
		const userInfo = await createUserWithEmailAndPassword(auth, email, password).then((firebaseUser) => {
			return firebaseUser
		})
		
		const docuRef = await doc(firestore, `/users/${userInfo.user.uid}`)
			...
			<div>
				<div className="flex flex-col items-center justify-center gap-5 p-5 rounded-lg shadow-lg w-fit">
					...
					<form onSubmit={handleSubmit} className="flex flex-col gap-5 w-fit " >
						...
						{
							isReg ? <label className="flex gap-3" >
								Rol
						...
						<input className="p-2 rounded-md shadow-md cursor-pointer bg-green hover:bg-ivory" type="submit" value={isReg ? "Registrar" : "Iniciar sesión"} />
					</form>
